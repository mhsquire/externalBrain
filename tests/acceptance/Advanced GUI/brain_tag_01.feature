Feature: The program makes a queue of the files on the system by location or by query
  Description: A queue of files is created from the database if the file is tagged with a timecode then it is inserted
  in order of that timecode with the newest virgin cards mixed in.
