Feature: The program allows the user to apply a dup tag when going through files
  ...
  Background:
    Given that the user has created a collection


  Scenario:
    When the user clicks on a file in the collection
    Then the file is marked with a tag tentatively and selected for tagging
  ...
  Scenario:
    ...
    Then the box is filled with the users selected name alphanumeric

  Scenario:
    When the user clicks enter next to the tag box
    Then all selected files are tagged in the database
